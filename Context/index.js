...
const PATIENT_APOINMENT_FEE = process.env.NEXT_PUBLIC_PATIENT_APPOINTMENT_FEE;
  ...
  const ADD_MEDICINE = async (medicine) => {
    try {
      ...
      if (address) {
        ...
        if (transaction.hash) {
          setLoader(false);
          notifySuccess("Medicine Registrations complete");
  ...
  const ADD_DOCTOR = async (doctor) => {
    try {
      ...
      if (address) {
        ...
        if (transaction.hash) {
          setLoader(false);
          notifySuccess("Registrations complete");
          window.location.reload();
        }
      }
    } catch (error) {
  ...
  const APPROVE_DOCTOR_STATUS = async (_doctorId) => {
    try {
      ...
      if (address) {
        ...
        if (transaction.hash) {
          setLoader(false);
          notifySuccess("Registrations Approve complete");
          setReCall(reCall + 1);
        }
      }
    } catch (error) {
